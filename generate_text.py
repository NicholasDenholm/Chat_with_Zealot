...
class GenerateText:
    ...
    def generate_text(model, seed, sequence_length, num_generated_chars, char_to_idx, idx_to_char):
        ...
        '''# Generate num_generated_chars characters
        for _ in range(num_generated_chars):
            output = model(input_tensor)  # Unpack both output and hidden state
            
            predictions = output[:, -1, :]  # Get the predictions for the next character
            prob_dist = torch.nn.functional.softmax(predictions, dim=-1)  # Apply softmax to get probabilities
            next_char_idx = torch.multinomial(prob_dist, 1).item()  # Sample the next character
            
            # Check if the predicted index is within the valid range
            next_char_idx = next_char_idx % len(idx_to_char)  # Wrap around if index is out of bounds
    ...
    @staticmethod
    def generate_text_2(model, seed, sequence_length, num_generated_chars, char_to_idx, idx_to_char):
        model.eval()
        input_seq = []
        ...
        for char in seed:
            if char in char_to_idx:
                ...
                # Handle missing character (e.g., replace with a space or a special token)
                print(f"Warning: Character '{char}' not found in vocabulary. Replacing with space.")
                input_seq.append(char_to_idx[' '])  # Replace with space or any other token
        
        # Proceed with the text generation logic
        input_tensor = torch.tensor(input_seq).unsqueeze(0)  # Add batch dimension
    ...
    @staticmethod
    def generate_text_1(model, seed, sequence_length, num_generated_chars, char_to_idx, idx_to_char):
        model.eval()
